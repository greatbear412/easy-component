...
module.exports = async component => {
  spinner.start('读取配置文件'); // read config
  ...
  spinner.stop(); // File check: Confirm which file to work with.
  ...
  const script = new Script(config.script, targetFile.script, FileService.getFileType(targetFile.script)); // template-check
  ...
  console.log(); // Component confirm: Confirm which component to work with for React/Vue.
  ...
  console.log(); // Parse and Transform and Generate.
  // Require parseHtml first in case working with React/Vue.

  template.parse(); // In Angular, template and script are in different files.
  ...
  const templateDataForJsxElement = {
    ...
    attrs: template.optional
  };
  script.parse(templateDataForJsxElement); // generate
...
class Script extends GenerateObject {
  ...
  parse(parseHtmlData) {
    ...
    const {
      ...
      plugins: [insertPlugin(parseScriptData, parseHtmlData)],
      generatorOpts: {
        decoratorsBeforeExport: true
      },
...
function getComponentsToWorkWith(sourceCode) {
  ...
  transformSync(sourceCode, {
    ...
    plugins: [getComponentPlugin(componentList)],
    sourceType: 'unambiguous'
  });
